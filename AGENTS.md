...
- **Pronunciation does not go through Web Speech.** Firefox ships no voices of its own and can only speak with one installed in the OS, so it has no Japanese voice unless the user installed a speech pack — and on Windows those land in the `Speech_OneCore` registry hive, which Firefox does not read. Chrome and Edge bundle their own voices and hide this entirely. Cards are therefore pronounced from clips committed under `public/audio/ja/`, resolved through `app/data/audio-manifest.ts`; `speechSynthesis` is only the fallback for text with no clip. Adding or editing a card means running `npm run audio`. Do not "simplify" [speech.ts](app/lib/speech.ts) back to synthesis-only — that is the bug this replaced.
